...
本文档在同一文件内包含 Phase 2 **产品/执行计划** 与 **SwiftUI → React 逐项迁移清单**（简体中文）。涵盖核心 UI 重建——将 Chat、Sidebar、Settings、Workspace 从 SwiftUI 全部用 React 重写，同时包含 "Spaces" 到 "Project" 的产品概念升级、每个 Protobuf service 迁移至 oRPC 的计划，以及一行行将 SwiftUI View 映射到 React 组件的迁移清单。清单部分可直接跳转：[第二部份 · 迁移清单](#part-2-migration-checklist-zh)。
